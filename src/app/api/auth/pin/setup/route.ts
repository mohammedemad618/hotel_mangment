...
import { pinSetupSchema } from '@/lib/validations';
    ...
    try {
        ...
        if (!rate.ok) {
            return NextResponse.json(
                { error: 'Too many PIN setup attempts, try again later' },
                {
                    status: 429,
        ...
        if (!requiresPinVerification(user.role)) {
            await clearPinChallengeCookie();
            return NextResponse.json({ error: 'PIN setup is not available for this role' }, { status: 403 });
        }
        ...
        if (
            ...
            return NextResponse.json({ error: 'PIN is already configured for this account' }, { status: 409 });
        }

        user.mfaSecret = await hashPin(parsed.data.pin);
