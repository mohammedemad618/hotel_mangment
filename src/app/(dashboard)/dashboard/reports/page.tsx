...
const statusLabels: Record<string, { label: { ar: string; en: string }; tone: string }> = {
    paid: { label: { ar: 'مدفوع', en: 'Paid' }, tone: 'badge-success' },
    partial: { label: { ar: 'جزئي', en: 'Partial' }, tone: 'badge-warning' },
    pending: { label: { ar: 'معلق', en: 'Pending' }, tone: 'badge bg-white/10 text-white/60' },
...
export default function ReportsPage() {
    ...
    const maxTrendRevenue = useMemo(() => Math.max(...trends.map((tRow) => tRow.revenue || 0), 1), [trends]);
    ...
    return (
        <div className="space-y-7">
            <div className="page-hero flex flex-col gap-5">
                <div className="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-white">{t(lang, 'التقارير', 'Reports')}</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        <select value={months} onChange={(e) => setMonths(Number(e.target.value))} className="input-compact min-w-[148px]">
                ...
                <div className="relative z-10 border border-white/10 rounded-2xl p-4 bg-white/[0.02]">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                        <div>
                            <h3 className="text-sm font-semibold text-white">
                                {t(lang, 'تصدير Excel حسب التاريخ', 'Excel Export by Date')}
                            </h3>
                            <p className="text-xs text-white/50 mt-1">
                                {t(lang, 'حدد الفترة الزمنية المطلوبة للتصدير.', 'Choose the date range for export.')}
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                            <input type="date" value={exportFromDate} onChange={(e) => setExportFromDate(e.target.value)} className="input-compact min-w-[145px]" />
                            <input type="date" value={exportToDate} onChange={(e) => setExportToDate(e.target.value)} className="input-compact min-w-[145px]" />
                            <button type="button" onClick={exportExcel} className="btn-primary" disabled={exportLoading}>
                                <Download className="w-4 h-4" />
                                {exportLoading ? t(lang, 'جارٍ التصدير...', 'Exporting...') : t(lang, 'تصدير Excel', 'Export Excel')}
                            </button>
                        </div>
                    </div>
                    {exportError ? (
                        <div className="mt-3 p-3 bg-danger-500/10 border border-danger-500/20 rounded-xl text-danger-500 text-sm">
                            {exportError}
                        </div>
                    ) : null}
                </div>
            ...
            {error ? (
                <div className="p-4 bg-danger-500/10 border border-danger-500/20 rounded-xl text-danger-500 text-sm">{error}</div>
            ) : null}

            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <div className="stat-card">
                    <p className="text-sm text-white/50">{t(lang, 'إيرادات الفترة', 'Period Revenue')}</p>
                    <p className="mt-2 text-2xl font-bold text-success-500">{formatCurrency(periodTotals.revenue)}</p>
                    {monthRevenueTrend ? (
                        <div className={`mt-2 flex items-center gap-2 text-xs ${monthRevenueTrend.isUp ? 'text-success-400' : 'text-danger-400'}`}>
                            ...
                            {t(lang, `${monthRevenueTrend.isUp ? 'تغير إيجابي' : 'تغير سلبي'} ${monthRevenueTrend.percent}%`, `${monthRevenueTrend.isUp ? 'Up' : 'Down'} ${monthRevenueTrend.percent}%`)}
                        </div>
                    ) : null}
                </div>
                <div className="stat-card">
                    <p className="text-sm text-white/50">{t(lang, 'مدفوعات الفترة', 'Period Paid')}</p>
                    <p className="mt-2 text-2xl font-bold text-primary-300">{formatCurrency(periodTotals.paid)}</p>
                    <div className="mt-3 h-2 rounded-full bg-white/10 overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-primary-500 to-accent-500" style={{ width: `${periodCollectionRate}%` }} />
                    </div>
                    <div className="mt-2 text-xs text-white/40">{t(lang, `نسبة التحصيل ${periodCollectionRate}%`, `Collection rate ${periodCollectionRate}%`)}</div>
                </div>
                <div className="stat-card">
                    <p className="text-sm text-white/50">{t(lang, 'المبالغ المستحقة', 'Outstanding')}</p>
                    <p className="mt-2 text-2xl font-bold text-warning-500">{formatCurrency(periodTotals.outstanding)}</p>
                </div>
                <div className="stat-card">
                    <p className="text-sm text-white/50">{t(lang, 'إجمالي الحجوزات', 'Total Bookings')}</p>
                    <p className="mt-2 text-2xl font-bold text-white">{periodTotals.bookings}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="card p-6 lg:col-span-2">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-semibold text-white">{t(lang, 'اتجاه الإيرادات والتحصيل', 'Revenue & Collection Trend')}</h2>
                        <span className="text-xs text-white/50">{t(lang, `${months} أشهر`, `${months} months`)}</span>
                    </div>
                    {loading ? (
                        <div className="flex justify-center py-14"><div className="spinner w-8 h-8" /></div>
                    ) : trends.length === 0 ? (
                        <div className="text-center text-white/60 py-14">{t(lang, 'لا توجد بيانات للعرض', 'No data to display')}</div>
                    ) : (
                        <>
                                    ...
                                    return (
                                        <div key={trend.month} className="flex-1 min-w-0 flex flex-col items-center gap-2">
                                            <div className="w-full max-w-[56px] h-44 rounded-2xl border border-white/10 bg-white/5 px-2 flex items-end gap-1">
                                                <div className="w-1/2 rounded-lg bg-gradient-to-t from-primary-500 to-accent-500" style={{ height: `${revenueHeight}%` }} />
                                                <div className="w-1/2 rounded-lg bg-success-500/90" style={{ height: `${paidHeight}%` }} />
                                            </div>
                                            <p className="text-[11px] text-white/60 truncate">{formatMonthLabel(trend.month)}</p>
                                        </div>
                                    );
                                })}
                            </div>
                ...
                <div className="card p-6">
                    <h2 className="text-lg font-semibold text-white mb-4">{t(lang, 'حالة المدفوعات', 'Payment Status')}</h2>
                    <div className="space-y-3">
                        {[
                            ...
                            const status = statusLabels[item.key];
                            return (
                                <div key={item.key} className="surface-tile flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <span className="text-sm text-white/70 truncate">{status.label[lang]}</span>
                                    </div>
            ...
            <div className="card p-6">
                <h2 className="text-lg font-semibold text-white mb-4">{t(lang, 'تفاصيل الأشهر', 'Monthly Details')}</h2>
                {loading ? (
                    <div className="flex justify-center py-10"><div className="spinner w-8 h-8" /></div>
                ) : trends.length === 0 ? (
                    <div className="text-center text-white/60 py-10">{t(lang, 'لا توجد تفاصيل متاحة.', 'No details available.')}</div>
                ) : (
                    <div className="table-container shadow-card">
                        <table className="table">
                            ...
                            <tbody>
                                {trends.map((trend) => {
                                    const rate = trend.revenue > 0 ? Math.min(100, Math.round((trend.paid / trend.revenue) * 100)) : 0;
                                    return (
                                        <tr key={trend.month}>
                                            ...
                                            <td><span className="badge-primary">{rate}%</span></td>
                                        </tr>
                                    );
                                })}
                            </tbody>
